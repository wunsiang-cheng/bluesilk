...
def call_tool(s, c):
    try:
        ...
        status = f"🖱 {args.get('action')}" if name == "computer" else f"🔧 {args.get('command') or name}"
        draft(s, status)
        fn = {"shell": shell, "send": send, "computer": computer}.get(name)
...
def browser_view(msg):
    ...
    return isinstance(content, list) and content and content[0].get("text", "").startswith("[browser screenshot]")
    ...
    while True:
        ...
        if visuals:
            ...
            for text, image in visuals:
                content += [{"type": "text", "text": f"[browser screenshot]\n{text}"},
...
def session_data(s):
    """Persist conversations without embedding generated browser screenshots in JSON."""
    ...
    for message in messages:
        ...
        for part in message["content"]:
            if part.get("type") == "image_url":
                part.clear()
                part.update(type="text", text="[browser screenshot omitted; use computer observe for a current view]")
