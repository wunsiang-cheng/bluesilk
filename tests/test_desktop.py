...
class DesktopUnitTests(BluesilkTestCase):
    def test_action_rejects_unknown_action_before_touching_the_display(self):
        backend = self.bs.XDesktopBackend()
        with mock.patch.object(backend, "_x") as x:
            with self.assertRaisesRegex(ValueError, "unknown desktop action"):
                backend._action(self.bs.Session("alice", "Alice"), {"action": "format_disk"})
            with self.assertRaisesRegex(ValueError, "drag requires y, x2"):
                backend._action(self.bs.Session("alice", "Alice"), {"action": "drag", "x": 1, "y2": 4})
        x.assert_not_called()

    def test_actions_map_to_xdotool_and_return_a_screenshot(self):
        backend = self.bs.XDesktopBackend()
        calls = []
        fake_shot = lambda argv, **kw: self.bs.Path(argv[-1]).write_bytes(b"\xff\xd8\xffjpeg")

        with mock.patch.object(backend, "_x", side_effect=lambda *a, text=None: calls.append((a, text))), \
             mock.patch.object(self.bs.subprocess, "run", side_effect=fake_shot), \
             mock.patch.object(self.bs.subprocess, "check_output", return_value="1536 1024\n"), \
             mock.patch.object(self.bs.time, "sleep"):
            result = backend._action(self.bs.Session("alice", "Alice"), {"action": "right_click", "x": 10, "y": 20})
            backend._action(self.bs.Session("alice", "Alice"), {"action": "type", "text": "héllo"})
            backend._action(self.bs.Session("alice", "Alice"), {"action": "scroll", "delta_y": -50})
    ...
    def test_init_desktop_tool_stays_disabled_without_x11(self):
        with mock.patch.dict(os.environ, {"DISPLAY": ""}), mock.patch.object(self.bs, "log") as log:
            self.bs.init_desktop_tool()
        self.assertIsNone(self.bs.DESKTOP)
        self.assertFalse(any(t["function"]["name"] == "desktop" for t in self.bs.TOOLS))
        log.assert_called_once()
        with self.assertRaisesRegex(RuntimeError, "desktop control is unavailable"):
            self.bs.desktop(self.bs.Session("alice", "Alice"), action="observe")

    def test_init_desktop_tool_publishes_once_when_available(self):
        with mock.patch.dict(os.environ, {"DISPLAY": ":0"}), mock.patch.object(self.bs.shutil, "which", return_value="/usr/bin/x"):
            self.bs.init_desktop_tool()
            self.bs.init_desktop_tool()
        self.assertIsNotNone(self.bs.DESKTOP)
        self.assertEqual(sum(t["function"]["name"] == "desktop" for t in self.bs.TOOLS), 1)
        self.assertIn("`desktop` controls", self.bs.system_prompt(self.bs.Session("alice", "Alice")))
    ...
    def test_observe_captures_a_virtual_display(self):
        ...
        try:
            ...
            with mock.patch.dict(os.environ, {"DISPLAY": ":97"}):
                result = self.bs.XDesktopBackend()._action(self.bs.Session("alice", "Alice"), {"action": "click", "x": 5, "y": 5})
